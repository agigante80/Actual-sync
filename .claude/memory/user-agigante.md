...
- **Wants a version bump per shipped batch, and will ask twice if it is missed.** A manual bump on `development` means MINOR by the Git Workflow rules (manual patch-bumping is the listed anti-pattern), so a batch of fixes ships as e.g. 1.15.0 -> 1.16.0. An unreleased bump is superseded rather than re-bumped: 1.14.0 never reached `main` and simply never released.
- **Wants a code review after each fix, not one at the end** — asked for it explicitly ("remember to code review at the end of each"). Run the loop per fix and report the stopping reason.
- **Decides from pros/cons, then delegates the doing.** The repeated pattern is: review the open tickets against the code, post pros/cons plus a suggested solution on each, let him pick, then implement exactly what he picked. He picked against my recommendation at least once, so present the options honestly rather than steering.
