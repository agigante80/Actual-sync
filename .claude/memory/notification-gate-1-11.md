...
Originated from **#168** (external reporter @Soulplayer, closed as delivered). Verifying that report surfaced four further bugs, all fixed in the same train: **#171** (notifySync logged "sent" when every channel failed), **#172** (a dead Telegram token logged ERROR every 2s forever), **#173** (`/health` and `actual_sync_info` reported different versions), **#174** (`notifications.webhooks.telegram` had never delivered a message). **#175** removed the uncalled `notifyError()`. **#176** (dead MessageFormatter error-formatter family) was closed in **v1.11.4** — 285 lines deleted, and it produced the class-method gate now described in [[hermetic-guards]] and CLAUDE.md.
