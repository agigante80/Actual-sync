...
**How to apply:** before any hook-gated git push (or running the full suite) from the Bash tool, prefix with `export NVM_DIR="$HOME/.nvm"; . "$NVM_DIR/nvm.sh"; nvm use 22`. If `better-sqlite3` was already built under 18, `npm rebuild better-sqlite3` under 22 first. This is a local-env quirk only — CI (Node 22) is unaffected, so never "fix" the tests for it. See [[actual-sync-project-overview]].
