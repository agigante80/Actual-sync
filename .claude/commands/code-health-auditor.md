...
entry point or a surface a consumer reaches by name: the package's published exports, a CLI/job
entry the runner invokes, or a symbol a text-parsing guard reads. This is exactly what the `entry`
and `ignore` config in `knip.json` is for. Before filing an `unused-export`/`unused-file` finding,
confirm it is not such an entry point; if it is one Knip does not yet know about, prefer widening
`knip.json` over filing a removal.
